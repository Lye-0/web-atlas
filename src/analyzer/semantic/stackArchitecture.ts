...
export function addStackArchitecture(model:ArchitectureModel,input:SemanticInput,analysis:SemanticAnalysis):void {
  ...
  for(const resource of [...input.resources].sort((a,b)=>Number(a.attributes?.dictionaryStackId==='docker-compose')-Number(b.attributes?.dictionaryStackId==='docker-compose'))){
    ...
    if(role==='local'&&['firebase-emulator-suite','firebase-authentication','cloud-firestore','firebase-storage'].includes(stackId)){
      const defined=node.evidence.some(e=>/(?:^|\/)firebase\.json$/.test(e.path)),used=node.evidence.some(e=>/\.[cm]?[jt]sx?$/.test(e.path));
      node.attributes.providerOriginLabel=defined&&used?'設定・コードの対応確認':defined?'設定で定義':'コード側の接続設定';
      node.attributes.providerIdentityNote=defined&&used?'同じプロジェクト・接続先の対応を確認。定義側と利用側の根拠を保持しています。':project?'同じホスト・ポートだけでは別の使用文脈との同一性を確定しません。対象プロジェクトの指定と根拠を保持しています。':'コード側の対象プロジェクトが静的に未特定のため、設定側と同一実体とは確定していません。localは実行場所であり、配備先環境のdevelopmentを意味しません。';
    }
