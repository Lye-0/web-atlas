...
it('retains combined code/artifact deployment arrivals and their exact original stages',()=>{
 ...
 expect(line.target).toBe(simple.owners.get('production'));expect(line.target).not.toBe(simple.owners.get('api'));expect(simple.owners.get('local')).toBe(simple.owners.get('api'));expect(simple.relations.get(line.id)!.edgeIds).toEqual(['publish']);expect([...simple.relations.values()].some(r=>r.paths?.some(p=>p.nodeIds.join(',')==='build,asset,deploy'))).toBe(true);expect(JSON.stringify(graph)).toBe(before);
});
it('groups environments of one destination setting but preserves different deployment settings',()=>{
 ...
 expect(simple.owners.get('a')).toBe(simple.owners.get('b'));expect(simple.owners.get('other')).not.toBe(simple.owners.get('b'));expect(simple.units.get(simple.owners.get('a')!)!.anchorId).toBeUndefined();
