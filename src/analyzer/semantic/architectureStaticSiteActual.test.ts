...
const roots={'HEAVY-PLAY':'C:/Users/kawau/dev/Interactive_Art/HEAVY-PLAY',Chess:'C:/Users/kawau/dev/Web_Services/Chess','git-lines':'C:/Users/kawau/dev/Extension/git-lines','vehicle-management':'C:/Users/kawau/dev/Web_Services/vehicle-management'};
it.skipIf(!process.env.STATIC_SITE_REVIEW)('reads three original projects without executing any input code',async()=>{
 await initializeTestParser();const reports:{project:string;[key:string]:unknown}[]=[];
 for(const [project,root]of Object.entries(roots).filter(([name])=>!process.env.STATIC_SITE_PROJECT||name===process.env.STATIC_SITE_PROJECT)){
  ...
  const stage=process.env.STATIC_SITE_REVIEW;
  await writeFile(`.cache/static-site/${project}.${stage}.store.json`,JSON.stringify(store));await writeFile(`.cache/static-site/${project}.${stage}.model.json`,JSON.stringify(model));
  await writeFile(`.cache/static-site/${project}.${stage}.simple.json`,JSON.stringify(simple,(_k,v)=>v instanceof Map?[...v]:v));
 ...
 const reportPath=`.cache/static-site/${process.env.STATIC_SITE_REVIEW}.report.json`;let previous:typeof reports=[];if(process.env.STATIC_SITE_PROJECT)try{previous=JSON.parse(await readFile(reportPath,'utf8'));}catch{/* first selected run */}await writeFile(reportPath,JSON.stringify([...previous.filter(r=>!reports.some(n=>n.project===r.project)),...reports],null,2));
},300000);
