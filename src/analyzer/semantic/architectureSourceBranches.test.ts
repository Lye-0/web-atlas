...
it.each([['x','review-east'],['renamed','任意の公開環境']] as const)('aligns sources and keeps real forks/joins after renaming %s', (prefix,env)=>{
 ...
 expect(simple.graph.nodes.filter(n=>n.attributes.simpleStage==='source'&&n.attributes.simplePlacement==='route').map(n=>simple.graph.architectureView!.positions2d!.get(n.id)!.x)).toEqual([0,0,0]);
...
it.skipIf(!process.env.SOURCE_BRANCHES)('checks three original inputs, old comparison, alignment and conservation',async()=>{
 ...
 for(const project of ['vehicle-management','git-lines','web-atlas']){
  ...
  const sources=simple.graph.nodes.filter(n=>n.attributes.simpleStage==='source'&&n.attributes.simplePlacement==='route');for(const n of sources)expect(simple.graph.architectureView!.positions2d!.get(n.id)!.x).toBe(0);
