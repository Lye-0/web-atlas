...
it.skipIf(!process.env.CONTENT_REVIEW)('checks all content choices against the three fixed project models',async()=>{
 const report=[];
 const previous=process.env.RANGE_REVIEW?JSON.parse(readFileSync('.cache/range-layout/actual-before.json','utf8')):process.env.PRESET_REVIEW?JSON.parse(readFileSync('.cache/preset-polish/actual-before.json','utf8')):undefined;
 for(const project of ['vehicle-management','git-lines','web-atlas']){
  ...
  const ranges=choices.filter(c=>c.id!=='all').map(choice=>{const range=architectureContentRange(model,choice.id)!;
   ...
   if(process.env.RANGE_REVIEW){
    ...
    expect(range.graph.nodes.map(n=>n.id)).toEqual(baseline.nodes.map((n:{id:string})=>n.id));expect(range.graph.edges.map(e=>({id:e.id,source:e.source,target:e.target,kind:e.kind,confidence:e.confidence,evidence:e.evidence.length}))).toEqual(baseline.edges);
   }
 ...
 writeFileSync(process.env.RANGE_REVIEW?'.cache/range-layout/actual-after.json':process.env.PRESET_REVIEW?'.cache/preset-polish/actual-after.json':'.cache/content-review/actual.json',JSON.stringify(report,null,2));
},180000);
