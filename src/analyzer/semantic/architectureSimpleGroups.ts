...
export interface SimpleGroup {id:string;label:string;reason:string;category:'destination'|'resources'|'unconfirmed';targetIds:string[]}
...
export function simpleStructuralGroups(model:SemanticGraph,allowed:ReadonlySet<string>){
 ...
 const destinations=new Set(model.edges.filter(e=>e.kind==='flow-deploys').map(e=>e.target));
 for(const n of model.nodes){if(!allowed.has(n.id))continue;
  if(destinations.has(n.id)&&n.architecture?.kind==='execution-config'){
   ...
   const id=`architecture-simple:destination:${JSON.stringify(owner&&config?[owner,config,n.attributes.executionPlace??'']:['original',n.id])}`;
   result.set(n.id,{id,label:`${byId.get(owner)?.label??n.label}：公開・配信先`,category:'destination',reason:'公開する指定の到着先。環境別の実行・配信構成を保持した表示集合',targetIds:owner?[owner]:[]});
  }
 ...
 for(const ids of destinationBuckets.values()){const environments=ids.map(id=>JSON.stringify([...(byId.get(id)!.architecture?.environments??[])].sort()));if(new Set(environments).size===environments.length)continue;for(const id of ids){const group=result.get(id)!;result.set(id,{...group,id:`${group.id}:${id}`,label:`${byId.get(id)!.label}：公開・配信先`});}}
