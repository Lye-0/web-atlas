...
it.skipIf(!process.env.GENERALITY_COMPARE)('compares original IDs, relations, stable regression input and cached projections',async()=>{
 const old=await import(/* @vite-ignore */resolve('.cache/generality/baseline-src/src/analyzer/semantic/architectureSimple.ts')),oldProjection=await import(/* @vite-ignore */resolve('.cache/generality/baseline-src/src/analyzer/semantic/architectureProjection.ts')),results=[];
 for(const project of ['Chess','git-lines','vehicle-management']){
  const before:SemanticGraph=JSON.parse(readFileSync(`.cache/generality/${project}.before.model.json`,'utf8')),after:SemanticGraph=JSON.parse(readFileSync(`.cache/generality/${project}.after.model.json`,'utf8')),nodeIds=new Set(after.nodes.map(n=>n.id)),edges=new Map(after.edges.map(e=>[e.id,e]));
 ...
 writeFileSync('.cache/generality/comparison.json',JSON.stringify(results,null,2));
},120000);
