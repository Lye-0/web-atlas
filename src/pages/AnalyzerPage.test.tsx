...
describe.each(['architecture', 'workspace', 'command', 'dependencies'] as const)('%s common search and camera contract', view => {
  ...
  if(view==='command')it('keeps the command selector visible and hides search in fullscreen',async()=>{
    ...
    const fullscreen=host.querySelector('.analyzer-controls-workspace.is-fullscreen')!;expect(fullscreen.contains(entry)).toBe(true);expect(search.closest('.analyzer-workspace-search')?.hasAttribute('hidden')).toBe(true);
    const value=entry.options[0]!.value;await act(async()=>{entry.value=value;entry.dispatchEvent(new Event('change',{bubbles:true}));});

    await act(async()=>host.querySelector<HTMLButtonElement>('[aria-label="全画面を終了"]')!.click());expect(host.querySelector('[aria-label="Command Flowのentry script"]')).toBe(entry);expect(entry.value).toBe(value);expect(search.closest('.analyzer-workspace-search')?.hasAttribute('hidden')).toBe(false);
  });
