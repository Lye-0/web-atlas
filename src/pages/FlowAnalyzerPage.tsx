...
export default function FlowAnalyzerPage({ view }: { view: SemanticExplorerViewId }) {
  ...
  const stageDisplays = useMemo(() => {
    ...
    if(simple)for(const node of stageGraph.nodes){const display=displays.get(node.id)!,purpose=String(node.attributes.simpleSupportPurpose??'');if(purpose){display.location=[purpose,display.location].filter(Boolean).join(' · ');display.disambiguation=[purpose,display.disambiguation].filter(Boolean).join(' · ');}}
    return displays;
