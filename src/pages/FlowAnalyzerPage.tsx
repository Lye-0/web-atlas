...
export default function FlowAnalyzerPage({ view }: { view: SemanticExplorerViewId }) {
  ...
  const stageDisplays = useMemo(() => {
    ...
    if(simple)for(const node of stageGraph.nodes){const display=displays.get(node.id)!,purpose=String(node.attributes.simpleSupportPurpose??'');if(purpose){display.location=[purpose,display.location].filter(Boolean).join(' · ');display.disambiguation=[purpose,display.disambiguation].filter(Boolean).join(' · ');}if(node.attributes.simpleCategory==='destination'){display.location=`公開先の設定 · ${node.attributes.simpleEnvironmentLabel}`;display.disambiguation=display.location;}if(Array.isArray(node.attributes.simpleRows))display.tooltip+='\n'+node.attributes.simpleRows.join('\n');}
    return displays;
