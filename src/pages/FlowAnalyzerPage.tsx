...
import {simpleResourceEndpoints} from '../analyzer/semantic/architectureSimpleAnnotations';
...
export default function FlowAnalyzerPage({ view }: { view: SemanticExplorerViewId }) {
  ...
  const stageDisplays = useMemo(() => {
    ...
    if(simple)for(const node of stageGraph.nodes){const display=displays.get(node.id)!,purpose=String(node.attributes.simpleSupportPurpose??'');
    ...
    if(simple&&selectedEdge)for(const match of simpleResourceEndpoints(simple,selectedEdge)){const display=displays.get(match.node.id);if(display){const labels=match.members.map(n=>[architectureEnvironmentContext(n).label,n.attributes.executionPlace].filter(Boolean).join(' / '));display.matchedEndpoint=`対応 ${match.members.length}対象：${labels.slice(0,2).join('、')}${labels.length>2?' ほか':''}`;display.disambiguation=display.matchedEndpoint;display.location=display.matchedEndpoint;display.tooltip+='\n'+match.labels.join('\n');}}
    return displays;
  }, [view, stageGraph.nodes, searchDisplays, byId,simple,selectedEdge]);
    ...
    <section className="analyzer-shell" aria-labelledby="analyzer-view-title">
      ...
      <div {...architectureGesture.bindings} ref={view==='architecture-map'?undefined:fullscreen.root} className={`analyzer-workspace semantic-flow-workspace${simple?' is-simple-overview':''}${session.detailOpen && (selected || selectedEdge || inspectedRequests) ? ' has-detail' : ''}${fullscreen.isFullscreen ? ' is-fullscreen' : ''}`}
        ...
        {<SemanticFlowStage loading={Boolean(store&&!analysis&&!error)} key={`${view}:${state.scanVersion}:${contentKey}`} graph={stageGraph} explorer={explorer} nodeDisplays={stageDisplays} mode={flow.mode} direction={options.direction} selectedIds={selectedIds} selectedEdgeId={selectedEdge?.id} matchIds={matchIds} focus={focus}
          ...
          contentDescription={simple?<section className="architecture-content-note" aria-label="簡易全体の説明"><strong>簡易全体</strong><p>原本・定義 → 用途別の操作・成果物 → 起動・公開・適用先</p><p>{architectureBase?.scopeId?'現在地と外側の文脈':'プロジェクト全体'}：{stageGraph.nodes.length}表示要素・{stageGraph.edges.length}表示線{simple.referenceEdges?.length?`・参照注記 ${simple.referenceEdges.length}束`:''}</p><details><summary>範囲・集計の詳細</summary><p>現在のフィルターで読める元対象{architectureBase?.allowed.length}件。表示要素は実体数ではありません。元関係の端点を説明単位へまとめ、同じ説明単位内の関係は詳細に保持しています。未特定要求も要求元ごとの内訳で確認できます。</p></details></section>:contentRange&&contentChoice&&<ArchitectureContentSummary current={stageGraph} choice={contentChoice} nodes={graph.nodes.length} edges={graph.edges.length} total={wholeGraph.nodes.length} emptyScope={contentEmpty} emptyFiltered={!stageGraph.nodes.length} onProject={navigation.project} onAll={()=>changeContent('all')}/>}
