...
it('separates original correspondence and serving target from the primary HTTP peer',async()=>{
 vi.stubGlobal('IS_REACT_ACT_ENVIRONMENT',true);const app=fixtureNode('source-app','application'),runtime=fixtureNode('runtime','execution-config');runtime.attributes.logicalOwnerId=app.id;
 const make=(id:string,source:string,target:string,kind:string)=>({id,source,target,kind,label:'same label',confidence:'source' as const,views:['architecture-map' as const],evidence:[{path:'settings.json',line:4,endLine:4,start:12,end:20,description:'config'}],details:{structural:true,architectureOrigin:'architecture' as const}});
 const simple=architectureSimpleOverview(prepareArchitectureScope({view:'architecture-map',nodes:[app,runtime],edges:[make('definition',app.id,runtime.id,'flow-definition'),make('serves',runtime.id,app.id,'flow-serves'),make('http',app.id,runtime.id,'http-request')]})),host=document.createElement('div'),root=createRoot(host);document.body.append(host);
 try{await act(async()=>root.render(<ArchitectureSimpleDetail simple={simple} node={simple.graph.nodes.find(n=>n.id===simple.owners.get(runtime.id))} sources={{}} onShowAll={()=>{}} onOpen={()=>{}} canOpen={()=>false} onClose={()=>{}}/>));expect(host.textContent).toContain('1対応先 · 2元関係');expect(host.textContent).toContain('対応する論理定義・配信対象');expect(host.textContent!.indexOf('概要・主な役割')).toBeLessThan(host.textContent!.indexOf('原本・コードとの対応'));expect(host.querySelectorAll('section')[1]?.textContent).toContain('source-app');expect(host.textContent).toContain('対応する原本・根拠を確認：2元関係');expect(host.textContent).toContain('外側との関係：1件');const peers=[...host.querySelectorAll('section')].find(s=>s.querySelector('h4')?.textContent==='主な構成上の相手')!;expect(peers.textContent).toContain('HTTPリクエスト');expect(peers.textContent).not.toContain('論理定義と構成の対応');}
...
it('limits correspondence preview by target and keeps same-name targets and every original relation reachable',async()=>{
 vi.stubGlobal('IS_REACT_ACT_ENVIRONMENT',true);const app=fixtureNode('app','application'),code=fixtureNode('code','code-definition');code.attributes.logicalOwnerId=app.id;
 const targets=Array.from({length:9},(_,i)=>{const n=fixtureNode(`target-${i}`,'execution-config');n.label='same name';n.attributes.logicalOwnerId=app.id;n.evidence=[{path:`settings-${i}.json`,line:1,endLine:1,start:0,end:1,description:'setting'}];return n;});
 const edges=targets.flatMap(n=>['flow-definition','flow-configures'].map(kind=>({id:`${kind}:${n.id}`,source:kind==='flow-definition'?app.id:code.id,target:n.id,kind,label:kind,confidence:'source' as const,views:['architecture-map' as const],details:{structural:true},evidence:n.evidence})));
 const simple=architectureSimpleOverview(prepareArchitectureScope({view:'architecture-map',nodes:[app,code,...targets],edges})),host=document.createElement('div'),root=createRoot(host),show=vi.fn();document.body.append(host);
 try{
  await act(async()=>root.render(<ArchitectureSimpleDetail simple={simple} node={simple.graph.nodes.find(n=>n.id===simple.owners.get(app.id))} sources={{}} onShowAll={show} onOpen={()=>{}} canOpen={()=>false} onClose={()=>{}}/>));
  const section=[...host.querySelectorAll('section')].find(s=>s.querySelector('h4')?.textContent==='原本・コードとの対応')!;
  expect(section.textContent).toContain('9対応先 · 18元関係');expect(section.textContent).toContain('ほか3対応先');expect(section.querySelectorAll('button')).toHaveLength(6);expect(section.textContent).toContain('settings-0.json');
  await act(async()=>section.querySelector('button')!.click());expect(show).toHaveBeenCalledWith('target-0');
  const d=section.querySelector('details')!;await act(async()=>{d.open=true;d.dispatchEvent(new Event('toggle'));});expect(section.textContent).toContain('18元関係');expect(section.querySelector('input[type="search"]')).not.toBeNull();expect(section.textContent).toContain('コードを使用する構成');
 }finally{await act(async()=>root.unmount());host.remove();vi.unstubAllGlobals();}
});
