...
it('exposes original intermediate IDs and evidence only on explicit disclosure',async()=>{
 ...
 const simple=architectureSimpleOverview(prepareArchitectureScope(model)),host=document.createElement('div'),root=createRoot(host),show=vi.fn();document.body.append(host);
 try{
  await act(async()=>root.render(<ArchitectureSimpleDetail simple={simple} edge={simple.graph.edges[0]} sources={{'sample.ts':'value'}} onShowAll={show} onOpen={()=>{}} canOpen={()=>false} onClose={()=>{}}/>));
  ...
  await open('元の関係・段階');await open('解析モデルの元関係');expect(host.textContent).toContain('middle-id');expect(host.textContent).toContain('original-id');expect(host.textContent).toContain('source marker');
