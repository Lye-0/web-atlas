...
// Native click detail identifies a double click, but both resolved node IDs must
// agree. Never reinterpret a second click on a control or another canvas point.
export function useArchitectureNodeGesture(context: string, enabled: boolean, canOpen: (id: string) => boolean, open: (id: string) => void, select: (id: string) => void) {
  ...
  const onNodeClick = (id: string, event: ArchitectureNodeClick, selectOverride?: () => void) => {
    ...
    cancel();
    if (event.button !== 0 || event.detail > 0 && down.current?.moved) return;
    if (enabled && first?.id === id && first.context === context && event.detail === 2
      && event.timeStamp - first.event.timeStamp <= 750
      && Math.hypot(event.clientX - first.event.clientX, event.clientY - first.event.clientY) <= 6) {
      if (canOpen(id)) open(id);
      return;
    }
  ...
  return {
    ...
    bindings: enabled ? {
      ...
      onPointerCancelCapture: () => { if (down.current) down.current.moved = true; cancel(); },
      onKeyDownCapture: cancel,
      onWheelCapture: cancel,
      onContextMenuCapture: cancel,
      onClickCapture: (event: MouseEvent) => {
        if (!(event.target instanceof Element) || !event.target.closest('[data-architecture-node-id], canvas')) cancel();
      },
    } : {},
  };
}
