...
import {simpleCorrespondencePeers,simpleResourceEndpoints,simpleResourceMemberLabel} from '../../analyzer/semantic/architectureSimpleAnnotations';
...
export function ArchitectureSimpleDetail({simple,node,edge,sources,onShowAll,onOpen,canOpen,onClose,onSelectSummary,onShowRelation,query=''}:{simple:ArchitectureSimple;node?:SemanticNode;edge?:SemanticEdge;sources:Record<string,string>;onShowAll:(id:string)=>void;onOpen:(id:string)=>void;canOpen:(id:string)=>boolean;onClose:()=>void;onSelectSummary?:(id:string)=>void;onShowRelation?:(id:string)=>void;query?:string}){
 ...
 const annotationIds=useMemo(()=>new Set((simple.referenceEdges??[]).filter(e=>e.source===node?.id||e.target===node?.id).flatMap(e=>simple.relations.get(e.id)?.edgeIds??[])),[simple,node]);
 const primaryExternal=useMemo(()=>inventory.external.filter(e=>!annotationIds.has(e.id)),[inventory.external,annotationIds]);
 ...
 const peers=useMemo(()=>node?simplePeerGroups(simple,node.id,primaryExternal).filter(p=>!tools.some(tool=>tool.id===p.id)):[],[simple,node,tools,primaryExternal]);
 ...
 const references=useMemo(()=>simple.original.edges.filter(e=>annotationIds.has(e.id)&&e.kind==='flow-invokes'),[simple,annotationIds]);
 const correspondences=useMemo(()=>simple.original.edges.filter(e=>annotationIds.has(e.id)&&e.kind!=='flow-invokes'),[simple,annotationIds]);
 const correspondencePeers=useMemo(()=>simpleCorrespondencePeers(correspondences,new Set(unit?.members??[]),nodes),[correspondences,unit,nodes]);
 ...
 return <aside className="analyzer-detail-panel is-module-detail semantic-detail semantic-flow-detail architecture-detail architecture-simple-detail" aria-label="構成の詳細">
  ...
  {node&&<ArchitectureStaticSiteInfo node={original??node} sources={sources}/>}
  {node?.attributes.providerOriginLabel&&<section className="simple-overview-section"><h4>{String(node.attributes.providerOriginLabel)}</h4><p>{String(node.attributes.providerIdentityNote??'')}</p></section>}
  {correspondencePeers.length>0&&<section className="simple-overview-section"><h4>原本・コードとの対応</h4><small>{correspondencePeers.length}対応先 · {correspondences.length}元関係</small>{correspondencePeers.slice(0,6).map(peer=><div key={peer.id}><p><button onClick={()=>onShowAll(peer.id)}>{peer.node.label}</button></p><small>{peer.labels.join('・')}{correspondencePeers.some(other=>other.id!==peer.id&&other.node.label===peer.node.label)?` · ${simpleMemberCaption(peer.node)}`:''}</small></div>)}{correspondencePeers.length>6&&<p>ほか{correspondencePeers.length-6}対応先。内訳で検索できます。</p>}<Disclosure title={`対応する原本・根拠を確認：${correspondences.length}元関係`}>{()=> <RelationList edges={correspondences} nodes={nodes} sources={sources} onShowRelation={onShowRelation}/>}</Disclosure></section>}
  ...
  {primaryExternal.length>0&&<Disclosure title={`${unit?'外側との関係':'元の関係・段階'}：${primaryExternal.length}件`}>{()=> <RelationList edges={primaryExternal} nodes={nodes} sources={sources} onShowRelation={onShowRelation}/>}</Disclosure>}
