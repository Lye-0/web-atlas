...
初期値「全体」は従来の正規モデル、投影、配置処理をそのまま使う。追加の「環境から見る」「経路から見る」は解析後のモデルを読む任意の表示範囲であり、解析結果や階層の再定義ではない。候補はarchitectureEnvironmentContextの区分とtool-operationのpurpose、実在する両端を持つ実行時関係から生成する。モデルにない経路は候補にしない。
