...
通常の案内は完全な選択名、短い目的説明、内容全体の対象/関係数と全体への導線を示す。長名は近接した案内に折り返し、hover依存にしない。現在の階層・条件の投影件数とは分け、元対象/集合/ソース箇所の厳密な定義は開閉できる説明に置く。最新の意味・表示・復元検証は[プリセットの仕上げ記録](tab10-preset-polish-review-20260917.md)を参照。
