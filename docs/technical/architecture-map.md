...
- `useArchitectureNodeGesture`はワークスペースで最初のクリックのIDを保持し、同じ位置の2回目をcaptureで処理する。詳細欄の出現で別要素がポインターの下へ来ても、別選択や二重移動を起こさない。ドラッグ、wheel、キー、別visitで取り消す。3Dはタブ10だけclick時に選択し、他Viewのpointerupを維持する。
...
3Dのラベル優先度は現在の階層から固定的に求める。直下は「内部」、内部または構成全体の境界と直接つながる外側は「接続先」、その他は「周辺」。外側の選択で追加された外側同士の描画線を、この分類の根拠に使わない。文字サイズ・ウェイト・上辺の印と説明を使い分け、選択・hover／focus・明示関係の対象だけ一時的に読みやすくする。解除後は元の区分へ戻す。分類は表示上の属性で、自動省略の所属・閾値・対象数・座標を変更しない。
