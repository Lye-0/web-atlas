...
2Dの環境・分類名は囲いの境界近くに直接表示する。スクリーン座標の13px文字と最大2行を使い、見出し・ノード・上部操作領域・下部凡例・内訳パネルを避けて、可視境界に沿う候補を安定ID順で選ぶ。離れた見出しは短い補助線で境界に結ぶ。画面外の囲いを中央へ固定しない。全文はクリック/Enterで開ける。ノード座標・意味上の所属・カメラは変更しない。

3Dは既存の重要点・選択ラベル優先を維持し、環境名の候補を投影した囲いの辺・頂点から選ぶ。囲い生成・DOMラベルの所属索引には共通のsemanticFlowRegionIdentityを使い、明示flowEnvironmentをexplorerの所属IDで上書きしない。回転時は境界の投影と補助線へ追従し、囲いOFFでは名前・補助線も消える。長名は2行表示し、キーボードフォーカスでも全文を確認できる。
...
最新の確認結果は[環境名の直接表示と分類説明の検証](tab10-direct-headings-review-20260917.md)を参照。前回の[環境見出し・相手要約の検証](tab10-headings-peer-summary-review-20260916.md)は変更前の記録であり、見出し方式と詳細の情報配分は今回更新した。
