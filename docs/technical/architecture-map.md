...
## 環境見出し・定義位置・相手要約（2026-09-16）

2Dの環境名は、現在地の下に高さを予約した折り返し可能な見出し領域へ表示する。安定した環境ID順の番号を囲い側の補助線・番号と対応させる。番号はズームに追従して画面上の文字サイズを保ち、他の番号とノードを避ける。ナビゲーション領域はクリップで保護する。ノード座標・意味上の所属は変更しない。3Dは既存の衝突回避付きラベルと囲い設定を維持する。
...
確認結果と制約は[環境見出し・相手要約の検証](tab10-headings-peer-summary-review-20260916.md)を参照。
