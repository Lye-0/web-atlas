...
- **Layout**: `layout.ts`のTab 5分岐から`spatialPacking.ts`の決定的なshelf packingを使用します。子Regionのサイズと親のheading / insetから包含boundsを作り、密なDirectoryのModule列数は件数から決めます。選択・Camera・LODではLayoutを変更しません。空の展開Setは初期の全展開であり、最初のtoggleで明示的な展開Setへ変換します。
...
- **Selection / edges**: 未選択では依存線を出しません。Module選択はincoming / outgoingの直接関係、Directory / Package選択は境界をまたぐ実際のModule間の関係を表示します。各Fact edgeのID・source・targetをそのまま保ち、同じFile pairの複数のimport宣言も個別の線にします。表示対象の階層に存在しないendpointはDirectoryの代替点へ置き換えません。import先は青、import元は琥珀色で、Detailの見出しも対応する色を持ちます。矢印はimportする側から読み込まれる側へ向きます。検索は候補の強調表示までで、選択は明示的なクリックで行います。背景クリックまたはEscで選択を解除します。pan開始ではSVG pathへのnative focusとtext dragを抑止し、keyboard focusは線自体を強調します。
- **LOD**: Moduleの占有範囲は全距離で固定し、Farで12×8の点へ置き換える処理は廃止しました。画面上の高さ18px未満ではファイル名を省略し、3px未満ではModule形状をRegionの面表示へまとめます。Region headingはズームに応じて文字・PKG / DIR・件数・展開記号を同じ比率で拡大縮小します（倍率の平方根、0.7〜1.8倍）。名前と件数の内容は距離で切り替えず、幅80px・高さ28pxを文字倍率で補正した領域を目安に表示します。横幅を自領域内に制限し、最初のFileより上へ配置します。選択Moduleの名前は遠距離でも別のバッジに保持します。選択によるCardの拡大や配置変更はしません。
...
Stack Mapは主要StackのScope帰属、Architecture Mapは配置とソース上の関係から分類した責務・Subsystemを示します。Package Dependencyは直接dependency宣言を示し、それぞれ別の投影を使用します。
