...
Semantic Regionの回帰は [`src/analyzer/analyzer.test.ts`](../../src/analyzer/analyzer.test.ts) と [`src/analyzer/session.test.ts`](../../src/analyzer/session.test.ts) で、Scope Nodeを生成しないこと、Regionのchild IDs / boundary ports / Project → Region endpoint、Region検索・filter・選択、Region boundsと子Node配置、旧`stack-scope:*` / `scope:*`選択のRegion移行を検証します。Region surfaceがpass-throughで、headingとchild Nodeだけがrouting obstacleになる契約は [`src/analyzer/edgeRouting.test.ts`](../../src/analyzer/edgeRouting.test.ts) のRegionケースで維持します。

実装時の検証結果:

- `pnpm test`: 9 test files / 85 tests passed（Stack MapのProject → Scope / Stack Usage投影・Scope normalization・Desktop Application集約・empty Scope pruning・root tooling分離、Declaration / Usage / Scope Evidence、source import、config-bound Scope attribution、single-packageの複数Usage Scope、Analyzerの既存回帰24 tests、Edge Routingの32 tests、Readability-aware RoutingとStructural Fan-out Route Composition、Geometry-aware Fan-out Direction Selection、Semantic Regionのmodel / layout / endpoint / obstacle / session回帰を含む）
