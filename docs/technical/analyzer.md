...
- **Region headings / controls**: Directory / Package名は塗りのない見出しと下線、DIR / PKG表記でFile Cardと区別します。自領域の最初のFileより上へ配置し、選択・展開操作を保持します。件数、方向filter、集約、選択解除、Camera controlsはCanvas外のtoolbarへ配置します。Canvasとtoolbarは一定のworkspace高さを分け合い、ResizeObserverは現在のCanvas stage要素だけを監視してDOM操作層とWebGLのviewportを一致させます。
...
- **Selection / aggregation**: 未選択では依存線を出しません。Module選択はincoming / outgoingの直接関係、Directory / Package選択はその範囲と外部との関係を表示します。import先は青、import元は琥珀色、範囲内部は緑で、文字付き凡例・件数・方向filterも表示します。矢印は常にimportする側から読み込まれる側へ向きます。高degreeは方向・相手Region単位にまとめ、「Directoryで集約」を解除すると個別接続を表示します。集約ラベルは「N件を集約」と明記し、Fact / Evidence / Detailは省略しません。Tab 5の検索は候補の強調表示までで、選択は明示的なクリックで行います。検索語を残して選択解除しても再選択しません。pan開始ではSVG pathへのnative focusとtext dragを抑止し、keyboard focusは線自体を強調します。
- **LOD**: Moduleの占有範囲は全距離で固定し、Farで12×8の点へ置き換える処理は廃止しました。画面上の高さ18px未満ではファイル名を省略し、3px未満ではModule形状をRegionの面表示へまとめます。Region headingは画面上の幅100px・高さ32pxを目安に表示し、全体の構造・件数を伝えます。選択Moduleの名前は遠距離でも別のバッジに保持します。選択によるCardの拡大や配置変更はしません。
- **Interaction / performance**: `spatialInteraction.ts`が入力をrequestAnimationFrameへ集約し、操作中はCameraと透明な操作overlayの単一affine transformを更新します。静止後にviewportとラベルの表示範囲を更新し、Session保存をdebounceします。新規gesture・Fit・Reset・Focusは古いtimerを取り消します。投影用Cameraの軸をmodel単位にcacheし、全Cardに対する二重の衝突判定をなくしました。ラベル衝突判定はheadingと集約ラベルだけに限定します。panでは矢印のinstance bufferも更新しません。Wheelは非passive listenerでページscrollを抑え、カーソル位置を中心に拡大縮小します。Card上からのdrag、矢印キー、Shift＋矢印キーの微調整、＋ / −、Home、Escape、DirectoryのEnterに対応します。
...
- `pnpm test`: 9 test files / 98 tests passed（Stack MapのProject → Scope / Stack Usage投影・Scope normalization・Desktop Application集約・empty Scope pruning・root tooling分離、Declaration / Usage / Scope Evidence、source import、config-bound Vite / Vitest / TypeScript attribution、single-packageの複数Usage Scope、Scope Promotion / usage-only非昇格 / nested Region hierarchy / recursive bounds / search-filter保持、Workspace FlowのRoot Package contains、Analyzerの既存回帰、Edge Routingの39 tests、Readability-aware RoutingとStructural Fan-out Route Composition、Geometry-aware Fan-out Direction Selection、Project → top-level Region fan-out、Semantic Regionのmodel / layout / endpoint / obstacle / session回帰を含む）
