...
5. **Module Dependency** — local / workspace source fileを1 Module Factとして検出し、Importer → Imported Moduleの静的`imports` EdgeだけをDirectory-aware Spatial Mapへ投影します。Package AreaとDirectoryはselectable Semantic Region、source fileだけがNodeです。既存2D rendererとは分離しています。Layout座標は x=right / y=screen-like down / z=elevation で、[`layoutToThreePoint()`](../../src/analyzer/spatialCoordinates.ts)だけが Three の `(x, up, depth=+layout.y)` へ変換します。Camera は visible world bounds の中心を lookAt し、Orthographic frustum / near-far も同じ bounds から決めます。World Spatial ModelはThree.js Orthographic Camera（浅いpitch + yaw）でscreenへ投影し、[`src/analyzer/spatialCoordinates.ts`](../../src/analyzer/spatialCoordinates.ts)の`projectSpatialPoint()` / `spatialCameraPose()`がPlaneとProjected Graph Layerのsingle source of truthです。R3F側は同じposeを[`src/analyzer/spatialThreeCamera.ts`](../../src/analyzer/spatialThreeCamera.ts)でCameraへ適用します。Module Card / Heading / Pillはscreen-facingのまま、Edge terminalはprojected Card / Region perimeterです。通常時はsame-directoryのlocal Module EdgeとRegion aggregateだけを出し、cross-directory / cross-packageの個別Module Edgeは常時表示しません。Module選択時だけdirect incidentへ解像し、unrelated Edgeは出しません。high-degreeは総incidentではなくincoming / outgoingとcounterpart Region単位でPresentation aggregationし、少数方向とvisibleな少数exact relationはCard-to-Cardのまま残します。Directory / Package aggregateのendpointはRegion boundaryだけを使い、child Module Cardへ接続しません。表示するcompact / full aggregateにはcounterpart・direction・countを対応させ、匿名stubは描画しません。Far FitはPackage planeに加えてscreen-facing Package heading boundsをinsetへ含め、Project headingとRoot Packageを混同しません。
