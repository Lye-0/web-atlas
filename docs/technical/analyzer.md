...
- `pnpm test`: 9 test files / 88 tests passed（Stack MapのProject → Scope / Stack Usage投影・Scope normalization・Desktop Application集約・empty Scope pruning・root tooling分離、Declaration / Usage / Scope Evidence、source import、config-bound Vite / Vitest / TypeScript attribution、single-packageの複数Usage Scope、Scope Promotion / usage-only非昇格 / nested Region hierarchy / recursive bounds / search-filter保持、Analyzerの既存回帰、Edge Routingの32 tests、Readability-aware RoutingとStructural Fan-out Route Composition、Geometry-aware Fan-out Direction Selection、Semantic Regionのmodel / layout / endpoint / obstacle / session回帰を含む）
