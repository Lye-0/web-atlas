...
The Data Flow function entrance prioritizes declared/named values, parameters, primary call/await results and return/termination boundaries. Literal values, source uses, actual argument slots and auxiliary expressions remain behind a counted fine-expression disclosure. Selected or explicitly focused fine objects remain primary, and scope-specific disclosure state survives mode/View changes independently of the 3D setting. This is presentation only: every occurrence remains in canonical analysis, search and operation input detail.

The shared Data Flow display helper supplies concise syntax-based names to blocks/cards, 3D labels, search results and detail headings. It distinguishes source roles such as formal parameter with owner/index, use location, actual slot, operation, result, field access and termination. Comments cannot change the syntactic delimiter scan. The complete original expression, owner and source range remain available in detail and the copyable code block. Names do not infer storage/authentication meaning. The accepted names/detail presentation in Views 6–7 remains compatible. Explicit multi-hop relations retain intermediate visibility and protect their canonical endpoints in 3D; the shared display projection is never used to infer a new route.

Data Model field rows respond to the structure panel's own inline size. Wide panels use the existing semantic table; narrow panels stack each name, type and qualities at full available width. Type identifiers remain intact, compound types can wrap at syntax delimiters, and actual overflow exposes a full-type/code-copy disclosure. Optionality, nullability, readonly and recorded visibility remain separate properties, with absent information left unknown. Class constructor properties, union fields and storage/schema fields use the same layout; zero fields and unexpanded structure remain distinct.

Union structures begin with their candidate count and concise list. A candidate ID uses model ID, candidate ordinal and original Evidence ranges, so duplicate discriminator values and missing discriminators cannot merge candidates. Recorded literal properties can identify a candidate; otherwise a type name or short structural description is used. Each structural candidate opens its own fields, complete definition and Evidence. Literal-only unions and enums keep value lists without empty tables. Per-View `modelOpenChoiceIds` preserves opened candidates across model and mode changes. A field-aware search selection opens matching candidates, and an unambiguous field match or explicit field jump selects its exact stable field ID. A model-name-only query does not open every candidate.

Views 8–9 use the [shared 3D display aggregation contract](semantic-analyzer.md#shared-3d-display-aggregation). Data Flow can aggregate by role, function, file and directory; coarser mixed-role collections have explicit breakdowns. Data Model candidates retain code/validation/storage domain separation. Automatic OFF leaves 2D disclosure/navigation and manual display state intact, and restores all automatically grouped point identities at their original coordinates.
